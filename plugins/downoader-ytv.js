...
const limit = 80;
const handler = async (m, { conn, args, isPrems, isOwner }) => {
  if (!args || !args[0]) throw 'Uhm... urlnya mana?';
  
  let chat = global.db.data.chats[m.chat];
  const isY = /y(es)/gi.test(args[1]);
  
  const { thumbnail, video: _video, title } = await youtubedlv2(args[0])
  ...
  if (!isY && !isLimit) {
    await conn.sendFile(m.chat, thumbnail, 'thumbnail.jpg', `
*YOUTUBE*
