...
export default async function handler(m, { conn, args, usedPrefix, command }) {
  ...
  try {
    const apiURL = `https://api.betabotz.org/api/download/ttslide?url=${args[0]}&apikey=AgatZApi`;
...
handler.command = handler.help = ['tiktokslide', 'ttslide', 'slide', 'ttimg', 'tiktokimg'];
